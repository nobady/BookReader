...
public class JavaBookModel extends BookModelImpl {
    ...
    JavaBookModel(Book book) {
        super(book);
        myInternalHyperlinks = new CachedCharStorage(32768, Paths.cacheDirectory(book.getIndex()), "links");
        BookTextModel = new ZLTextWritablePlainModel(null, book.getLanguage(), 1024, 65536, Paths.cacheDirectory(book.getIndex()) , "cache", myImageMap);
    }
    ...
    public ZLTextModel getFootnoteModel(String id) {
        ...
        if (model == null) {
            model = new ZLTextWritablePlainModel(id, Book.getLanguage(), 8, 512, Paths.cacheDirectory(Book.getIndex()), "cache" + myFootnotes.size(), myImageMap);
