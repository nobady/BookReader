...
abstract class AnimationProvider {
    ...
    void startAnimatedScrolling(int x, int y, float xVelocity, float yVelocity, int speed) {
        if (myMode != Mode.ManualScrolling) {
            return;
        }
        ...
        final float velocity = myDirection.IsHorizontal ? xVelocity : yVelocity;
        boolean forward = Math.abs(velocity) > ViewConfiguration.get(AppContext.getInstance()).getScaledMinimumFlingVelocity();
        ...
        if (myDrawInfos.size() > 1) {
            ...
            for (int i = 1; i < myDrawInfos.size(); ++i) {
                ...
                calcSpeed += (float) Math.sqrt(dX * dX + dY * dY) / Math.max(1, info1.Start - info0.Start);
            }
            calcSpeed /= myDrawInfos.size() - 1;
            calcSpeed *= duration;
            calcSpeed = Math.min(100, Math.max(15, calcSpeed));
        }
        myDrawInfos.clear();
        ...
        switch (myDirection) {
            case up:
            case rightToLeft:
                mySpeed = forward ? -calcSpeed : calcSpeed;
                break;
            case leftToRight:
            case down:
                mySpeed = forward ? calcSpeed : -calcSpeed;
